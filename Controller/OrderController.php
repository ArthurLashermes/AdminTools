...
use AdminTools\Form\OrderCreateForm;
use AdminTools\Util\Calc;
use AdminTools\Util\CriteriaSearchTrait;
...
use AdminTools\ModelOrder\Order;
    ...
    public function ajaxModalCreateAction(Request $request, ParserContext $parserContext, EventDispatcherInterface $eventDispatcher, RequestStack $requestStack, SecurityContext $securityContext)
    {
        ...
        if (!$form->hasError() && 'create' === $formValidate->get('action')->getData()) {
            ...
            try {
                ...
                if ((int) $orderStatusId >= 2) {
                    if ((int) AdminTools::getConfigValue(AdminTools::CONFIG_KEY_INVOICE_REF_TYPE) === 0) {
                        ...
                        $order->setInvoiceRef((int) AdminTools::getConfigValue(AdminTools::CONFIG_KEY_INVOICE_REF_INCREMENT));

                        AdminTools::setConfigValue(
                            AdminTools::CONFIG_KEY_INVOICE_REF_INCREMENT,
                            (int) AdminTools::getConfigValue(AdminTools::CONFIG_KEY_INVOICE_REF_INCREMENT) + 1
                        );
        ...
        if ($order->getId()) {
            ...
            return $this->render('admin-order-creation/ajax/order-create-modal', [
                ...
                'configNewCreditNoteStatusId' => AdminTools::getConfigValue(AdminTools::CONFIG_KEY_DEFAULT_NEW_CREDIT_NOTE_STATUS_ID),
                'configNewCreditNoteTypeId' => AdminTools::getConfigValue(AdminTools::CONFIG_KEY_DEFAULT_NEW_CREDIT_NOTE_TYPE_ID),
                'configPayedOrderMinimumStatusId' => AdminTools::getConfigValue(AdminTools::CONFIG_KEY_PAYED_ORDER_MINIMUM_STATUS_ID)
            ]);
        }
    }
